# Code adopted from https://github.com/nghorbani/body_visualizer/blob/master/src/body_visualizer/tools/vis_tools.py and https://github.com/nghorbani/body_visualizer/blob/master/src/body_visualizer/tools/mesh_tools.py
# Adhere to their license.
...
def render_smpl_params(bm, body_params, rot_body=None, color=Color.GREY, h=800, w=800):
    mv = MeshViewer(width=w, height=h, use_offscreen=True)
    mv.set_cam_trans([0, 0.5, 3.0])
    faces = bm.f.detach().cpu().numpy()

    v = bm(**body_params)[0].detach().cpu().numpy()
    ...
    for fIdx in range(T):
        ...
        mesh = trimesh.base.Trimesh(verts, faces, vertex_colors=num_verts * color.value)

        mv.set_meshes([mesh], 'static')

        images.append(mv.render())

    return np.array(images).reshape(T, w, h, 3)

def meshes_as_png(meshes, outpath=None, view_angles=[0, 180], h=800, w=800):

    mv = MeshViewer(h, w)
    mv.set_cam_trans([0, -.5, 1.75])
    images = np.zeros([len(meshes), len(view_angles), 1, w, g, 3])
...
def render_sequence(bm, body_params, rot_body=None, w=800, h=800, color=Color.GREY, fps=30, outpath=None):
    imgs = np.expand_dims(render_smpl_params(bm, body_params, rot_body, color, h, w), axis=(0,1))
    return imagearray2file(imgs, outpath=outpath, fps=fps)